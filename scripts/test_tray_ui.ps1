...
Invoke-Command 'startup-off' 'Startup disabled'
...
Invoke-Command 'startup-on' 'Startup enabled'
$runValue = Get-ItemPropertyValue -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Run' -Name 'AmirRazerLightingSwitch'
if ($runValue -notmatch 'RazerLightingSwitch.exe.+startup') { throw 'Startup enable failed' }
